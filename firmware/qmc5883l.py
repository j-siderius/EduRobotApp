...
class QMC5883L():
    ...
    def __init__(self, i2c, address = None, offset=21.0):
        self.i2c = i2c
        self.address = address if address else self.ADDR
    ...
    def reconfig(self):
        self.command[0] = (self.oversampling | self.range |
                           self.rate | self.mode)
        self.i2c.writeto_mem(self.address, QMC5883L.CONFIG,
                             self.command)
        time.sleep(0.01)
        self.command[0] = QMC5883L.CONFIG2_INT_DISABLE
        self.i2c.writeto_mem(self.address, QMC5883L.CONFIG2,
                             self.command)
        time.sleep(0.01)
    ...
    def ready(self):
        status = self.i2c.readfrom_mem(self.address, QMC5883L.STATUS, 1)[0]
    ...
    def read_raw(self):
        try:
            while not self.ready():
                print(".", end='')
                time.sleep(0.005)
            self.i2c.readfrom_mem_into(self.address, QMC5883L.X_LSB,
                                       self.register)
