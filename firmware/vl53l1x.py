...
class VL53L1X:
    ...
    def _write_register(self, address, data, length=None):
        if length is None:
            length = len(data)
        self._i2c.writeto(self._address, struct.pack(">H", address) + data[:length])

    def _read_register(self, address, length=1):
        self._i2c.writeto(self._address, struct.pack(">H", address))
        data = self._i2c.readfrom(self._address, length)
        return data
...
class DistanceManager:
    ...
    async def polling(self, websocket_message_broadcast):
        while True:
            # if self.vl53.data_ready:  # send update when new data is available
            await asyncio.sleep(1/10)  # update at 20Hz
            await websocket_message_broadcast(f"/front_distance/{self.vl53.distance}")
...
if __name__ == "__main__":
    ...
    while True:
        if distance.vl53.data_ready:
            print(distance.vl53.distance)
...
#             time.sleep(1.0 / vl53.timing_budget)
